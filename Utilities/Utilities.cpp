...
bool comparisonFirst(
    const pair<float, unsigned> &i_pair1, const pair<float, unsigned> &i_pair2
) {
 ...
 **/
float clip(
    const float i_value, const float i_min, const float i_max
) {
 ...
 * @return none.
 **/
void determineFactor(const unsigned i_n, unsigned &o_a, unsigned &o_b) {
  if (i_n == 1) {
    o_a = 1;
    o_b = 1;
    return;
