...
SMART_PARAMETER_INT(STEP1_SIZE, 3);
SMART_PARAMETER_INT(STEP2_SIZE, 5);
SMART_PARAMETER_INT(STEP1_SEARCHWIN, 7 * STEP1_SIZE());
SMART_PARAMETER_INT(STEP2_SEARCHWIN, 7 * STEP2_SIZE());
SMART_PARAMETER_INT(STEP1_NSIM, 50);
SMART_PARAMETER_INT(STEP2_NSIM, 40);
SMART_PARAMETER_FLOAT(STEP1_FLAT_THRESHOLD, 1.f);
SMART_PARAMETER_FLOAT(STEP2_FLAT_THRESHOLD, 1.f);
SMART_PARAMETER_FLOAT(STEP2_TAU0, 17.f);
...
void initializeNlbParameters(nlbParams &o_paramStep1,
  ...
  o_paramStep1.doPasteBoost = false;
  o_paramStep2.doPasteBoost = false;
 ...
 **/
int runNlBayes(std::vector<float> const &i_imNoisy,
...
#pragma omp parallel for schedule(dynamic, nbParts/nbThreads) \
    ...
    if (subBuild(o_imBasic, imBasicSub, p_imSize, imSizeSub, paramStep1.boundary) != EXIT_SUCCESS) {
      return EXIT_FAILURE;
    }
...
#pragma omp parallel for schedule(dynamic, nbParts/nbThreads) \
    ...
    if (subBuild(o_imFinal, imFinalSub, p_imSize, imSizeSub, paramStep2.boundary) != EXIT_SUCCESS) {
      return EXIT_FAILURE;
    }
...
void estimateSimilarPatchesStep1(
  ...
  partial_sort(distance.begin(), distance.begin() + nSimP, distance.end(), comparisonFirst);
